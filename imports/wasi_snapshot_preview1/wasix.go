...
func (m *Module) WasixSockOpen(ctx context.Context, family Int32, sockType Int32, protocol Int32, openfd Pointer[Int32]) Errno {
	...
	result, errno := m.WASI.SockOpen(ctx, wasi.ProtocolFamily(family), wasi.SocketType(sockType), wasi.Protocol(protocol), rightsBase, rightsInheriting)
	if errno != wasi.ESUCCESS {
		return Errno(errno)
	}
...
func (m *Module) WasixSockSendTo(ctx context.Context, fd Int32, iovecs List[wasi.IOVec], flags Int32, addr Pointer[wasixAddrPort], nwritten Pointer[Int32]) Errno {
	socketAddr, ok := m.wasixGetAddrPort(addr.Load())
	if !ok {
		return Errno(wasi.EINVAL)
	}
